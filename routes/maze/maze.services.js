...
class Maze {
  ...
  solveMaze(id, end, pos, prevmoves = "") {
    const directions = this.getDirections(id, pos);
    if (pos == end) {
      console.log("Found solution: "+prevmoves);
      return prevmoves;
    }
    let _this = this;
    directions.forEach(function(dir) {
      let movearray = prevmoves.split('');
      if (dir != _this.invert(movearray[movearray.length-1])) {
        let newpos = _this.getNewPos(pos, dir);
        _this.solveMaze(id, end, newpos, prevmoves+dir);
      }
  });
  }

  getDirections(id, coord) {
    const cell = this.mazes[id][coord[1]][coord[0]].split('');
    let directions = [];
    if (coord[0] != 5 && cell[3] != 1) {
      directions.push('r');
    }
    if (coord[1] != 0 && cell[2] != 1) {
      directions.push('u');
    }
    if (coord[0] != 0 && cell[1] != 1) {
      directions.push('l');
    }
    if (coord[1] != 5 && cell[0] != 1) {
      directions.push('d');
    }
    return directions;
  }
